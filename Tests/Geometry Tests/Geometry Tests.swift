...
// MARK: - Quantized Space Tests

/// A test quantized space with 0.01 quantum (same as PDF UserSpace)
enum TestQuantizedSpace: Quantized {
    typealias Scalar = Double
    static var quantum: Double { 0.01 }
}

/// Geometry using the test quantized space
typealias QGeometry = Geometry<Double, TestQuantizedSpace>

@Suite
struct QuantizedTests {
    @Test
    func `Quantize function rounds to quantum`() {
        // 0.01 quantum means values snap to 2 decimal places
        #expect(TestQuantizedSpace.quantize(1.234) == 1.23)
        #expect(TestQuantizedSpace.quantize(1.235) == 1.24)  // rounds up
        #expect(TestQuantizedSpace.quantize(1.236) == 1.24)
        #expect(TestQuantizedSpace.quantize(0.001) == 0.00)
        #expect(TestQuantizedSpace.quantize(0.005) == 0.01)  // rounds up at midpoint
    }

    @Test
    func `Quantum conversion works`() {
        // Verify the quantum(as:) method works correctly
        let q: Double = TestQuantizedSpace.quantum(as: Double.self)
        print("Quantum as Double: \(q)")
        #expect(q == 0.01)

        let qFloat: Float = TestQuantizedSpace.quantum(as: Float.self)
        print("Quantum as Float: \(qFloat)")
        #expect(qFloat == 0.01)
    }

    @Test
    func `Tagged init quantized`() {
        let value = Tagged<TestQuantizedSpace, Double>(quantized: 1.234, quantum: 0.01)
        #expect(value.rawValue == 1.23)
    }

    @Test
    func `X coordinate plus Width displacement is quantized`() {
        let x: QGeometry.X = .init(100.0)
        let width: QGeometry.Width = .init(21.8)

        // Adding should produce quantized result
        let result = x + width
        #expect(result.value == 121.8)

        // Now test with values that would accumulate error
        let x2: QGeometry.X = .init(84.0)
        let width2: QGeometry.Width = .init(21.8)
        let row1 = x2 + width2  // 105.8
        let row2 = row1 + width2  // Should be 127.6, not 127.60000000000001
        let row3 = row2 + width2  // Should be 149.4, not 149.40000000000003

        #expect(row1.value == 105.8)
        #expect(row2.value == 127.6)
        #expect(row3.value == 149.4)
    }

    @Test
    func `Y coordinate plus Height displacement is quantized`() {
        let y: QGeometry.Y = .init(100.0)
        let height: QGeometry.Height = .init(21.8)
    ...
    @Test
    func `Coordinate subtraction returns quantized displacement`() {
        let x1: QGeometry.X = .init(149.4)
        let x2: QGeometry.X = .init(84.0)
        let width: QGeometry.Width = x1 - x2

        #expect(width.value == 65.4)
    }

    @Test
    func `Displacement addition is quantized`() {
        let w1: QGeometry.Width = .init(21.8)
        let w2: QGeometry.Width = .init(21.8)
        let w3: QGeometry.Width = .init(21.8)

        let sum = w1 + w2 + w3
        #expect(sum.value == 65.4)
    }

    @Test
    func `Rectangle llx getter is quantized`() {
        // Create a rectangle with values that would cause precision issues
        ...
        // The llx getter should return a quantized value
    ...
    @Test
    func `Adjacent rectangle boundaries align exactly`() {
        // This is the core test: two adjacent rectangles should share exact boundary values
        // Simulating table cells where row 1 ends at row 2's start

        let startY: QGeometry.Y = .init(84.0)
        let rowHeight: QGeometry.Height = .init(21.8)

        // Row 1: y=84, height=21.8 -> ury = 105.8
        ...
        // Row 2 starts where row 1 ends
        let row2StartY = row1.ury
        ...
        // Row 3 starts where row 2 ends
        let row3StartY = row2.ury
        ...
        // Now create a "spanning" rectangle that should match row3's ury
        // This simulates a rowspan cell that was computed differently
        let spanHeight: QGeometry.Height = .init(65.4)  // 3 * 21.8
        let spanRect = QGeometry.Rectangle(
            x: .init(0),
            y: startY,
            width: .init(100),
            height: spanHeight
        ...
        #expect(row3.ury.value == spanRect.ury.value)

        // Print for debugging
        print("Row 1 ury: \(row1.ury.value)")
        print("Row 2 lly: \(row2.lly.value)")
        print("Row 2 ury: \(row2.ury.value)")
        print("Row 3 lly: \(row3.lly.value)")
        print("Row 3 ury: \(row3.ury.value)")
        print("Span ury:  \(spanRect.ury.value)")
    }
}
