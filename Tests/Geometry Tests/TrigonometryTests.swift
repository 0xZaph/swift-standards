...
struct TrigonometryTests {
    ...
    func radianTrigonometry() {
        let angle: Geometry<Double>.Radian = .piOver2
        #expect(abs(angle.sin - 1.0) < 1e-10)
        #expect(abs(angle.cos) < 1e-10)

        let angle45: Geometry<Double>.Radian = .piOver4
    ...
    func radianConstants() {
        ...
        #expect(abs(Geometry<Double>.Radian.piOver2.value - Double.pi / 2) < 1e-10)
        #expect(abs(Geometry<Double>.Radian.piOver4.value - Double.pi / 4) < 1e-10)
        #expect(abs(Geometry<Double>.Radian.piOver3.value - Double.pi / 3) < 1e-10)
        #expect(abs(Geometry<Double>.Radian.piOver6.value - Double.pi / 6) < 1e-10)
    }
    ...
    func radianToDegree() {
        ...
        let rad2: Geometry<Double>.Radian = .piOver2
    ...
    func affineTransformRotationRadian() {
        let transform = Geometry<Double>.AffineTransform.rotation(.piOver2)
    ...
    func vectorUnitAtAngle() {
        let v = Geometry<Double>.Vector.unit(at: .piOver4)
    ...
    @Test("Vector polar")
    func vectorPolar() {
        let v = Geometry<Double>.Vector.polar(length: 2.0, angle: .piOver6)
    ...
    func vectorRotation() {
        ...
        let rotated = v.rotated(by: .piOver2)
    ...
    func pointPolar() {
        let p = Geometry<Double>.Point.polar(radius: 2.0, angle: .piOver3)
    ...
    func pointRotationOrigin() {
        ...
        let rotated = p.rotated(by: .piOver2)
        #expect(abs(rotated.x) < 1e-10)
        #expect(abs(rotated.y - 1.0) < 1e-10)
    }
    ...
    func pointRotationCenter() {
        ...
        let rotated = p.rotated(by: .piOver2, around: center)
