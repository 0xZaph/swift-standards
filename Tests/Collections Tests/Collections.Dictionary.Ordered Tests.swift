...
@Suite("Collections.Dictionary.Ordered")
struct OrderedDictionaryTests {
    ...
    func initFromPairs() throws {
        let dict = try Collections.Dictionary.Ordered<String, Int>([
            ("a", 1),
            ("b", 2),
            ("c", 3)
        ])
    ...
    func initThrowsOnDuplicate() {
        #expect(throws: Collections.Dictionary.Ordered<String, Int>.Error.self) {
            _ = try Collections.Dictionary.Ordered<String, Int>([
                ("a", 1),
                ("b", 2),
                ("a", 3)  // Duplicate
            ])
    ...
    func initWithUniquingClosure() {
        let dict = Collections.Dictionary.Ordered<String, Int>(
            [("a", 1), ("b", 2), ("a", 10)],
    ...
    @Test("Equality")
    func equality() {
        var a = Collections.Dictionary.Ordered<String, Int>()
        a["x"] = 1
        a["y"] = 2

        var b = Collections.Dictionary.Ordered<String, Int>()
        b["x"] = 1
        b["y"] = 2

        var c = Collections.Dictionary.Ordered<String, Int>()
        c["y"] = 2
